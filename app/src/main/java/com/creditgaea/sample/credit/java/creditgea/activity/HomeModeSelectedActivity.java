...
public class HomeModeSelectedActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        context=HomeModeSelectedActivity.this;
        setContentView(R.layout.home_mode_screen);
        init();
    }
            ...
            case R.id.btnSave:
             /*   if(cb_time_jet.isChecked() || cb_time_nonjet.isChecked() ||
                ...
                if (tv_time_jet.getText().toString().equals("Total Time- 00:00:00") && tv_time_nonjet.getText().toString().equals("Total Time- 00:00:00")
                    ...
                    if (!tv_time_jet.getText().toString().equals("Total Time- 00:00:00")) {
                        netscore = AppConstants.score_home_electricity;
                    }
                    if (!tv_time_nonjet.getText().toString().equals("Total Time- 00:00:00")) {
                        netscore =AppConstants.score_home_mobile;
                    }
                    if (!tv_time_solar.getText().toString().equals("Total Time- 00:00:00")) {
                        netscore = AppConstants.score_home_solar;
                    }

                    if(AppConstants.getTravelLogs(context)==0)
                    {
                        AppConstants.saveModeToDb(context,getUsername,getDateEntered,"","Home","",""+netscore);

...
import com.creditgaea.sample.credit.java.chat.ui.activity.ChatActivity;
...
public class ChatAdapter extends RecyclerView.Adapter<ChatAdapter.NewMessageViewHolder> implements
    ...
    private void onBindViewNotificationHolder(NewMessageViewHolder holder, QBChatMessage chatMessage) {
         ...
         */
        if(chatMessage.getBody().startsWith("μ")){
            holder.ln_pay_container.setVisibility(View.VISIBLE);
            String [] payMessageReceive = chatMessage.getBody().split("μ");

            if(payMessageReceive.length==5){
                String payment = payMessageReceive[1];
                holder.tvMessageBody.setText("$"+payment);
                holder.tv_pay_status_message.setText("Received ");
            }else {
                String payment = payMessageReceive[1];
                ...
                if(type.equalsIgnoreCase("0")){
                    holder.tv_pay_status_message.setText("Received From "+userName);
                }else {
                    holder.tv_pay_status_message.setText("Request From "+userName);
                }
            }



            holder.tv_pay_status_message.setTextColor(Color.BLACK);
            holder.tvMessageBody.setPadding(0,3,0,5);

        }else {
            //  holder.tvMessageBody.setTextColor(Color.BLACK);
            holder.tvMessageBody.setPadding(0,3,0,16);
            holder.ln_pay_container.setVisibility(View.GONE);
        ...
        if(chatMessage.getBody().startsWith("μ")){
            ...
            if(payMessageReceive.length==5){
                ...
                if(isIncomingMessage){
                    ...
                    holder.tv_pay_status_message.setText("Sent ");
                }
            }else {
                String payment = payMessageReceive[1];
                String type = payMessageReceive[3];
                ...
                if(isIncomingMessage){
                    if(type.equalsIgnoreCase("0")){
                        ...
                        holder.tv_pay_status_message.setText("Request Received  ");
                    }
                }else {
                    ...
                    if(type.equalsIgnoreCase("0")){
                        ...
                        holder.tv_pay_status_message.setText("Request Sent");
                    }
                }
    ...
    public static class NewMessageViewHolder extends RecyclerView.ViewHolder {
        ...
        LinearLayout ln_pay_container = itemView.findViewById(R.id.ln_pay_container);
        TextView tv_pay_status_message = itemView.findViewById(R.id.tv_pay_status_message);
